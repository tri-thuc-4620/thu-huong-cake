...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">Chinh sua don hang <strong>#{{ $order->order_number }}</strong></h4>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('store_location_id') is-invalid @enderror" name="store_location_id">
                                ...
                                @foreach($stores as $store)
                                    <option value="{{ $store->id }}" {{ old('store_location_id', $order->store_location_id) == $store->id ? 'selected' : '' }}>{{ $store->name }}</option>
                                @endforeach
                            </select>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <input type="text" class="form-control form-control-sm bg-light" value="{{ $item->product_name ?? ($item->product->name ?? '') }}" readonly>
                                        <input type="hidden" name="items[{{ $index }}][product_id]" value="{{ $item->product_id }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control form-control-sm bg-light" value="{{ $item->cake_size ?? '' }}" readonly>
                                        <input type="hidden" name="items[{{ $index }}][cake_size_id]" value="{{ $item->cake_size_id ?? '' }}">
                                    </td>
                                    <td>
                                        <input type="text" class="form-control form-control-sm bg-light" value="{{ $item->cake_base ?? '' }}" readonly>
                                        <input type="hidden" name="items[{{ $index }}][cake_base_id]" value="{{ $item->cake_base_id ?? '' }}">
                                    </td>
                                    <td><input type="number" class="form-control form-control-sm" name="items[{{ $index }}][quantity]" value="{{ $item->quantity }}" min="1"></td>
                                    <td><input type="number" class="form-control form-control-sm" name="items[{{ $index }}][unit_price]" value="{{ $item->unit_price }}"></td>
                                    <td><input type="text" class="form-control form-control-sm bg-light" value="{{ number_format($item->quantity * $item->unit_price) }}" readonly></td>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="subtotal" value="{{ old('subtotal', $order->subtotal) }}" readonly>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Phi giao hang</label>
                        <input type="number" class="form-control @error('shipping_fee') is-invalid @enderror" name="shipping_fee" value="{{ old('shipping_fee', $order->shipping_fee) }}">
                        @error('shipping_fee')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('discount') is-invalid @enderror" name="discount" value="{{ old('discount', $order->discount) }}">
                        @error('discount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control fw-bold" name="total" value="{{ old('total', $order->total) }}" readonly>
                    </div>
                    <div class="mb-0">
