...
@section('title', 'Chinh sua trang')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0">Chinh sua trang</h4>
...
<div class="card">
    <div class="card-body">
        <form action="{{ route('admin.pages.update', $page ?? 1) }}" method="POST">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label for="title" class="form-label">Tieu de</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $page->title ?? 'Gioi thieu') }}">
                @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <div class="mb-3">
                <label for="slug" class="form-label">Slug</label>
                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug" value="{{ old('slug', $page->slug ?? 'gioi-thieu') }}">
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('layout') is-invalid @enderror" id="layout" name="layout">
                    <option value="default" {{ old('layout', $page->layout ?? 'default') == 'default' ? 'selected' : '' }}>Mac dinh</option>
                    <option value="sidebar" {{ old('layout', $page->layout ?? '') == 'sidebar' ? 'selected' : '' }}>Co sidebar</option>
                    <option value="full" {{ old('layout', $page->layout ?? '') == 'full' ? 'selected' : '' }}>Toan trang</option>
                </select>
